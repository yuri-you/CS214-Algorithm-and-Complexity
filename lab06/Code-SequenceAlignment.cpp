...
void editDistanceDP(string &X, string &Y, int xLow, int yLow, int xHigh, int yHigh, const int &delta, bool reverse, vector<int> &dist) {
	...
	*/
	if (reverse) {//reverse position
		vector<int> last;
		dist.assign(xHigh - xLow + 1, 0);
		for (int i = 0; i <= yHigh - yLow; ++i) {
			last.assign(dist.begin(), dist.end());
			for (int j = 0; j <= xHigh - xLow; ++j) {
				if (i == 0) {
					...
					if (j == 0)
						...
						int min1 = dist[j - 1] + delta;
						min1 = min(min1, last[j - 1] + alpha(X[xHigh-j], Y[yHigh -i]));
						min1 = min(min1, last[j] + delta);
						dist[j] = min1;
					}
				}
			}
			...
			for (int j = 0; j <= xHigh-xLow; ++j) {
				if (i == 0) {
					...
					if (j == 0)
						...
						int min1 = dist[j-1] + delta;
						min1 = min(min1, last[j - 1] + alpha(X[xLow + j - 1], Y[yLow + i - 1]));
						min1 = min(min1, last[j] + delta);
						dist[j] = min1;
					}
				}
...
int editDistanceDC(string &X, string &Y, int xLow, int yLow, int xHigh, int yHigh, const int &delta, vector<vector<int>> &points) {
	...
	editDistanceDP(X, Y, xLow, yLow, xHigh, yMid, delta, false, dist1);//positive direction
